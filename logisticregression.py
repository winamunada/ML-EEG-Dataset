...
print('Logistic regression classifier accuracy on test set: {:.4f}'.format(logreg.score(X_test, y_test)))

from sklearn.metrics import classification_report
print(classification_report(y_test, y_predlog))

y,levels = pd.factorize(df['specific.disorder'])
cnb = pd.crosstab(levels[y_test],levels[y_predlog])
fig, ax = plt.subplots(figsize=(15, 5))
nb = sns.heatmap(cnb, annot=True, fmt=".0f", ax=ax, cmap="Oranges")
nb.set(xlabel ="Prediction", ylabel = "Actual")
nb.set_title("Logistic regression", fontsize =15, weight='bold')
...
y_predbest = best_logreg.predict(X_test)

print('Logistic regression classifier accuracy on test set: {:.4f}'.format(best_logreg.score(X_test, y_test)))

from sklearn.metrics import classification_report
print(classification_report(y_test, y_predbest))

y,levels = pd.factorize(df['specific.disorder'])
cnb = pd.crosstab(levels[y_test],levels[y_predbest])
fig, ax = plt.subplots(figsize=(15, 5))
nb = sns.heatmap(cnb, annot=True, fmt=".0f", ax=ax, cmap="Oranges")
nb.set(xlabel ="Prediction", ylabel = "Actual")
nb.set_title("Logistic regression", fontsize =15, weight='bold')